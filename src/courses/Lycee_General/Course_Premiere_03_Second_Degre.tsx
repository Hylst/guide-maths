...
import { 
  ...
  FillInTheBlanks, FormulaBox, InteractiveExercise, AccordionFAQ
    ...
    <div className="max-w-5xl mx-auto pb-16">
      ...
      <Section title="📜 Les Racines (Les Formules de x)" icon="🗝️" color="slate">
        ...
        <div className="space-y-4 mt-6">
          <InfoBlock type="reminder" title="Rappel important : Le sommet S de la Parabole">
            Le sommet {"$S$"} de la parabole représentant le polynôme {"$f(x) = ax^2 + bx + c$"} a pour coordonnées {"$(\\alpha ; \\beta)$"} où :
            <div className="text-center my-2 font-mono text-sm">
              <MathComponent math="\alpha = -\frac{b}{2a} \quad \text{et} \quad \beta = f(\alpha) = -\frac{\Delta}{4a}" />
            </div>
            Le sommet correspond soit au maximum de la fonction (si a &lt; 0), soit au minimum (si a &gt; 0).
          </InfoBlock>

          <InfoBlock type="funfact" title="Le saviez-vous ? Les tablettes babyloniennes de 1800 av. J.-C.">
            Plus de 1500 ans avant que les Européens ne développent les écritures algébriques modernes, les scribes de Babylone savaient déjà résoudre des problèmes géométriques équivalents au second degré (par exemple, trouver la longueur et la largeur d'un terrain sachant sa surface et son périmètre) ! Ils utilisaient pour cela des méthodes géométriques de découpage et recollement d'aires.
          </InfoBlock>

          <InfoBlock type="info" title="Zoom sur : Les trajectoires de projectiles (Physique)">
            Dans le champ de pesanteur terrestre, tout objet lancé (un ballon de basket, un javelot, ou un oiseau en colère dans Angry Birds !) suit une trajectoire parabolique parfaite modélisée par une équation du second degré. Trouver où l'objet va toucher le sol revient exactement à chercher les racines (les intersections de la parabole avec l'axe horizontal) !
          </InfoBlock>
        </div>
      </Section>
      ...
      <Section title="❓ FAQ (Questions Fréquentes)" icon="HelpCircle" color="slate">
        <AccordionFAQ 
          items={[
            {
              question: "Pourquoi le coefficient 'a' ne peut-il jamais être égal à 0 dans le second degré ?",
              answer: "Si a = 0, le terme en ax² disparaît complètement. L'équation n'a alors plus de carré, elle devient bx + c = 0, ce qui est une simple équation du premier degré (une droite) ! La détection du second degré requiert expressément un coefficient 'a' non nul."
            },
            {
              question: "Que se passe-t-il concrètement si le discriminant Delta est négatif ?",
              answer: "Graphiquement, cela signifie que la parabole ne coupe jamais l'axe horizontal des abscisses. Elle reste soit entièrement au-dessus de l'axe (si elle sourit et a > 0), soit entièrement en-dessous (si elle est triste et a < 0). Dans les nombres réels (R), l'équation n'a donc aucune solution."
            },
            {
              question: "Pourquoi parle-t-on de 'racine double' lorsque Delta est nul ?",
              answer: "Lorsque Delta vaut exactement 0, les deux formules des racines x₁ et x₂ donnent le même résultat : -b / 2a. On dit que c'est une racine double car algébriquement, le polynôme peut se factoriser sous la forme a(x - x₀)², où le facteur (x - x₀) apparaît deux fois."
            }
          ]}
        />
      </Section>
