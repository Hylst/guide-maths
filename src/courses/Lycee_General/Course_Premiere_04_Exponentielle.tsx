    ...
    <div className="max-w-5xl mx-auto pb-16">
      ...
      <Section title="📜 Les Pouvoirs Magiques (Propriétés Algébriques)" icon="⚡" color="amber">
        ...
        <div className="space-y-4 mt-6">
          <InfoBlock type="reminder" title="Rappel de dérivation : Dérivée de e^u">
            En Première, on apprend aussi à dériver des fonctions composées avec l'exponentielle de la forme {"$e^{u(x)}$"}. La formule générale est :
            <div className="text-center my-2 font-mono text-sm">
              <MathComponent math="[e^{u(x)}]' = u'(x) \times e^{u(x)}" />
            </div>
            Par exemple, la dérivée de {"$e^{5x + 3}$"} est {"$5e^{5x + 3}$"}.
          </InfoBlock>

          <InfoBlock type="funfact" title="Le saviez-vous ? Le jeu d'échecs et la ruine du Roi">
            La légende raconte que l'inventeur du jeu d'échecs demanda pour seule récompense au roi de placer un grain de riz sur la première case, puis de doubler sur la suivante (2 grains, puis 4, puis 8...), et ainsi de suite jusqu'à la 64ème case. Le roi accepta en souriant d'une telle 'modestie'... avant de réaliser que la somme de cette progression géométrique vaut {"$2^{64} - 1$"}, soit plus de 18 milliards de milliards de grains de riz ! Cela dépasse largement toute la production mondiale de l'histoire humaine. C'est la force de la croissance exponentielle.
          </InfoBlock>

          <InfoBlock type="info" title="Zoom sur : La datation au Carbone 14 (Archéologie)">
            Lorsqu'un organisme vivant meurt, la proportion de Carbone-14 radioactif (un atome instable) qu'il contient diminue de façon continue à vitesse constante. C'est une décroissance exponentielle, modélisée par une équation de la forme {"$N(t) = N_0 \times e^{-kt}$"} (avec un exposant négatif). En mesurant le Carbone-14 restant dans un fossile, les archéologues déterminent précisément son âge !
          </InfoBlock>
        </div>
      </Section>
      ...
      <Section title="❓ FAQ (Questions Fréquentes)" icon="HelpCircle" color="slate">
        <AccordionFAQ 
          items={[
            {
              question: "D'où vient la lettre 'e' de la fonction exponentielle ?",
              answer: "La lettre 'e' a été choisie par le grand mathématicien suisse Leonhard Euler au XVIIIe siècle. Elle est parfois appelée 'nombre d'Euler'. C'est une constante irrationnelle universelle au même titre que π !"
            },
            {
              question: "Pourquoi dit-on que l'exponentielle transforme les sommes en produits ?",
              answer: "C'est sa propriété la plus bluffante : exp(a + b) = exp(a) × exp(b). Cette équation fonctionnelle unique permet de simplifier d'immenses calculs d'échelle en physique et économie."
            },
            {
              question: "Quelle est la différence fondamentale entre x² (puissance) et e^x (exponentielle) ?",
              answer: "Dans x², la base varie et la puissance est fixe. Dans e^x (l'exponentielle), c'est l'inverse : la base est fixe (le nombre e) et c'est la puissance (x) qui varie ! L'exponentielle e^x croît de manière infiniment plus rapide que n'importe quelle fonction puissance x^n lorsque x devient grand."
            }
          ]}
        />
      </Section>
