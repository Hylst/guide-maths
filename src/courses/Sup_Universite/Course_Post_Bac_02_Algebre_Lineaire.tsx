...
import LinearTransform from '../../components/interactive/LinearTransform';
    ...
    <div className="max-w-5xl mx-auto pb-16">
      ...
      <Section title="⚠️ Introduction : Plus Haut, Plus Loin" icon="🚀" color="emerald">
        ...
        <InfoBlock type="funfact" title="Le saviez-vous ?">
          L'algorithme originel de Google, <strong>PageRank</strong>, repose entièrement sur l'algèbre linéaire ! Il modélise le web mondial comme une immense matrice de transition de probabilités (avec des milliards de colonnes). Le "poids" ou l'importance de chaque page web correspond mathématiquement au vecteur propre de cette matrice associé à la valeur propre maximale 1.
        </InfoBlock>
      </Section>

      <Section title="⚖️ Familles de Vecteurs" icon="👨‍👩‍👧‍👦" color="indigo">
        <p className="mb-4">
          Dans un espace E, on peut choisir un petit ensemble de vecteurs qu'on appelle "famille". On les juge selon deux critères cruciaux.
        </p>
        ...
        <InfoBlock type="reminder" title="Rappel">
          Dans un espace vectoriel {"$E$"} de dimension finie {"$n$"}, toute famille libre de {"$n$"} vecteurs est automatiquement une base, et toute famille génératrice de {"$n$"} vecteurs est également une base. De plus, les coefficients d'un vecteur dans une base donnée sont uniques : ils définissent les coordonnées exactes du vecteur.
        </InfoBlock>
      </Section>
      ...
      <Section title="⚔️ Exercices Interactifs" icon="⚙️" color="amber">
        <InteractiveExercise
          ...
          steps={[
            <div className="bg-amber-50/50 dark:bg-amber-900/20 p-4 rounded-xl border border-amber-100 dark:border-amber-800/60 font-normal text-slate-900 dark:text-slate-100">
      ...
      <Section title="📜 Applications Linéaires & Noyau" icon="🏭" color="amber">
        ...
        <div className="my-4 text-center">
            <h4 className="font-bold text-amber-900 dark:text-amber-100">Le Théorème du Rang (Très Important)</h4>
        ...
        <InfoBlock type="info" title="Zoom sur : La Décomposition SVD (Valeurs Singulières)">
          L'un des sommets de l'algèbre linéaire pratique est la factorisation <strong>SVD</strong> (Singular Value Decomposition). Elle stipule que toute matrice rectangulaire peut se décomposer sous la forme {"$A = U \\Sigma V^T$"}. C'est la base de la compression d'image JPEG, du filtrage du bruit de signal, et du moteur de recommandation collaboratif (comme celui de Netflix).
        </InfoBlock>
      </Section>
      ...
      <Section title="🎮 Simulateur Spatial & Transformations 2D" icon="🕹️" color="slate">
        <LinearTransform />
