...
import StochasticProcessSim from '../../components/interactive/StochasticProcessSim';
    ...
    <div className="max-w-5xl mx-auto pb-16">
      ...
      <Section title="⚠️ Introduction : Le Temps Aléatoire" icon="🌩️" color="emerald">
        ...
        <InfoBlock type="funfact" title="Le saviez-vous ?">
          Une trajectoire de mouvement brownien passe une infinité de fois par TOUTES les valeurs réelles possibles dans n'importe quel intervalle de temps, si court soit-il ! Ses oscillations sont si fines que si on veut en calculer la dimension géométrique (dimension de Hausdorff), on obtient exactement 2. C'est un objet fractal parfait et continu.
        </InfoBlock>
      </Section>
      ...
      <Section title="⚖️ Les Processus de Markov & Mouvement Brownien" icon="⛓️" color="indigo">
        ...
        <InfoBlock type="reminder" title="Rappel : L'Espérance Conditionnelle">
          Dans les processus stochastiques, l'information acquise jusqu'à l'instant {"$t$"} est représentée formellement par une tribu {"$\\mathcal{F}_t$"}. L'espérance conditionnelle {"$E[X_{t+s} \\mid \\mathcal{F}_t]$"} correspond mathématiquement à la meilleure prévision possible de la valeur future {"$X_{t+s}$"} sachant tout l'historique disponible.
        </InfoBlock>
      </Section>
      ...
      <Section title="📜 La Formule Magique de Wall-Street : Itô" icon="📈" color="amber">
        ...
        <InfoBlock type="info" title="Zoom sur : L'Intégrale d'Itô">
          Le mouvement brownien n'étant pas à variation bornée, on ne peut pas définir l'intégrale {"$\\int_0^t H_s dW_s$"} au sens habituel de Riemann-Stieltjes. Elle est construite comme une limite dans l'espace Hilbertien {"$L^2(P)$"} de fonctions simples (par paliers). Sa propriété magique est de consigner une espérance nulle, faisant de l'intégrale d'Itô une <strong>martingale</strong> par rapport à sa filtration naturelle.
        </InfoBlock>
      ...
      <Section title="🎮 Testeur Stochastique (Visualisateur de Trajectoires)" icon="🕹️" color="slate">
        <StochasticProcessSim />
