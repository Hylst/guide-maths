...
import SeriesConvergence from '../../components/interactive/SeriesConvergence';
  ...
  return (
    <div className="max-w-5xl mx-auto pb-16">
      ...
      <Section title="⚠️ Introduction : L'Infini qui tient dans la main" icon="♾️" color="emerald">
        ...
        <InfoBlock type="funfact" title="Le saviez-vous ? (Le Paradoxe de Zénon)">
          Le célèbre paradoxe d'Achille et la Tortue, proposé par le philosophe grec Zénon au Ve siècle avant J.-C., affirmait que le coureur rapide Achille ne pourrait jamais rattraper une lente tortue s'il lui laissait une longueur d'avance. Zénon prétendait qu'Achille devait d'abord parcourir la moitié de la distance, puis la moitié de la distance restante, et ainsi de suite à l'infini. L'invention des séries géométriques a résolu ce paradoxe en prouvant qu'une <strong>somme infinie de durées peut être finie</strong> : {"$\\sum_{n=1}^{\\infty} (1/2)^n = 1$"} !
        </InfoBlock>
      </Section>
      ...
      <Section title="⚖️ La Différence entre Suite et Série" icon="🕵️" color="indigo">
        ...
        <div className="my-4 p-4 border border-rose-100 dark:border-rose-800/60 bg-rose-50/50 dark:bg-rose-900/20 rounded-xl text-rose-950 dark:text-rose-50 text-sm">
        ...
        <InfoBlock type="reminder" title="Rappel">
          Si la suite générale {"$u_n$"} ne tend pas vers 0, la série associée {"$\\sum u_n$"} diverge grossièrement de manière immédiate. Cependant, l'équivalent à 0 de {"$u_n$"} n'est <strong>pas une condition suffisante</strong> du tout, comme le démontre l'exemple de la série harmonique.
        </InfoBlock>
      </Section>
      ...
      <Section title="📜 La Série Harmonique et Riemann" icon="🏛️" color="amber">
        ...
        <div className="my-4 text-center p-4 bg-muted rounded-xl border border-border">
        ...
        <InfoBlock type="info" title="Zoom sur : Le Problème de Bâle et Euler">
          En 1735, Leonhard Euler acquit une célébrité immédiate en résolvant le "Problème de Bâle", à savoir : que vaut la somme exacte de la série convergente des inverses de carrés {"$\\sum_{n=1}^{\\infty} \\frac{1}{n^2}$"} ? Euler démontra de façon étourdissante qu'elle vaut exactement {"$\\frac{\\pi^2}{6}$"}. Plus tard, Riemann généralisera ce calcul sous la fameuse fonction zêta : {"$\\zeta(s) = \\sum_{n=1}^{\\infty} \\frac{1}{n^s}$"} dont la caractérisation des zéros non triviaux est l'un des sept problèmes du millénaire toujours ouverts.
        </InfoBlock>
      </Section>
      ...
      <Section title="🎮 Simulateur de Convergence de Séries" icon="🕹️" color="slate">
        <SeriesConvergence />
