...
import TopologicalSpaces from '../../components/interactive/TopologicalSpaces';
    ...
    <div className="max-w-5xl mx-auto pb-16">
      ...
      <Section title="⚠️ Introduction : Le Caoutchouc" icon="🍩" color="emerald">
        ...
        <InfoBlock type="funfact" title="Le saviez-vous ?">
          Le mathématicien Stefan Banach a prouvé qu'on pouvait couper une sphère solide en un nombre fini de morceaux et les réassembler pour obtenir DEUX sphères identiques et pleines, de même taille que l'originale ! C'est le <strong>paradoxe de Banach-Tarski</strong>. C'est mathématiquement rigoureux grâce à l'existence d'ensembles non-mesurables, mais physiquement impossible car la matière réelle est faite d'atomes indivisibles.
        </InfoBlock>
      </Section>
      ...
      <Section title="⚖️ Ouverts, Fermés et Frontières" icon="🚪" color="indigo">
        ...
        <InfoBlock type="reminder" title="Rappel">
          Un ensemble {"$A$"} est <strong>Fermé</strong> si et seulement si son complémentaire {"$E \\setminus A$"} est un <strong>Ouvert</strong>. L'adhérence {"$\\bar{A}$"} correspond au plus petit fermé contenant {"$A$"}, tandis que l'intérieur {"$A^{\\circ}$"} est le plus grand ouvert contenu dans {"$A$"}.
        </InfoBlock>
      </Section>
      ...
      <Section title="📜 La Continuité par les Ouverts" icon="🌊" color="amber">
        ...
        <InfoBlock type="info" title="Zoom sur : Les Espaces Métriques">
          La topologie usuelle sur les ensembles de nombres réels ou complexes repose sur la notion familière de distance {"$d(x,y) = |x - y|$"}. Tout ensemble muni d'une distance satisfaisant l'inégalité triangulaire s'appelle un <strong>espace métrique</strong>. Dans ces espaces, les voisinages et ouverts s'expriment très simplement à l'aide de boules ouvertes {"$B(x, \\epsilon) = \\{y \\in E \\mid d(x,y) < \\epsilon\\}$"}.
        </InfoBlock>
      </Section>
