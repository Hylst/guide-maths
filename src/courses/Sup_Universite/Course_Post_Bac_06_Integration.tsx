...
import RiemannSums from '../../components/interactive/RiemannSums';
  ...
  return (
    <div className="max-w-5xl mx-auto pb-16">
      ...
      <Section title="⚠️ Introduction : Oublie l'intervalle gentil [a, b]" icon="🌊" color="emerald">
        ...
        <InfoBlock type="funfact" title="Le saviez-vous ? (L'Intégrale de Lebesgue)">
          Pour intégrer des fonctions très discontinues ou sauvages (comme la fonction indicatrice des rationnels, qui vaut 1 si {"$x \\in \\mathbb{Q}$"} et 0 sinon), l'intégrale de Riemann échoue. Au début du XXe siècle, Henri Lebesgue a révolutionné l'analyse en proposant un nouveau découpage de l'aire : au lieu de couper l'axe des abscisses {"$x$"} en petits intervalles (Riemann), il découpa l'axe des ordonnées {"$y$"} en bandes horizontales et introduisit la notion de <strong>théorie de la mesure</strong>.
        </InfoBlock>
      </Section>
      ...
      <Section title="⚖️ Nature de l'intégrale : Converge ou Diverge ?" icon="🔭" color="indigo">
        ...
        <TipBanner type="info" title="L'Astuce des Équivalents">
          Comme pour les Séries Numériques, le but de l'étudiant est de se débarrasser des calculs immenses en d'accordant : "Ma fonction compliquée ressemble beaucoup à 1/x² en l'infini. Or je SAIS que 1/x² converge. Donc ma fonction compliquée converge ! " (C'est le théorème de Comparaison, valable UNIQUEMENT pour les fonctions positives f{">"}0).
        </TipBanner>
        ...
        <InfoBlock type="reminder" title="Rappel : Intégration par parties (IPP)">
          L'intégration par parties découle directement de la dérivée d'un produit de fonctions : {"$\\int u(x)v'(x)dx = u(x)v(x) - \\int u'(x)v(x)dx$"} ! C'est la ruse fondamentale pour simplifier des produits où l'un des termes s'annule par dérivation successive (comme les polynômes) ou se transpose facilement (comme les sinus/cosinus).
        </InfoBlock>
      </Section>
      ...
      <Section title="📜 Les Critères de Riemann (Les Mètres-Étalons)" icon="📏" color="amber">
        ...
        <InfoBlock type="info" title="Zoom sur : L'Intégrale de Gauss et la loi normale">
          L'une des intégrales les plus célèbres en mathématiques et en physique est l'intégrale de Gauss : {"$\\int_{-\\infty}^{+\\infty} e^{-x^2} dx = \\sqrt{\\pi}$"}. Bien que la fonction cloche d'une loi normale {"$e^{-x^2}$"} ne possède aucune primitive s'exprimant à l'aide de fonctions usuelles, la valeur exacte de son aire totale se calcule par un passage astucieux en coordonnées polaires en dimension 2. Elle est à la base de toute la statistique classique et de la courbe de distribution d'erreurs d'observation de Gauss.
        </InfoBlock>
      </Section>
      ...
      <Section title="🎮 Simulateur Spatial d'Approximation (Sommes de Riemann)" icon="🕹️" color="slate">
        <RiemannSums />
